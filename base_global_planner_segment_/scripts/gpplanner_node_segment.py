...
def publish_path_segment():
    ...
    if get_value.number_of_sequences!=0:
      ...
      for i in range(len(get_value.path_segment_publish)):
        ...
        if abs(euclidean_distance(get_value.start,get_value.pub_path[-1]))<get_value.threshold_distance_to_goal: 
...
if __name__ == "__main__":
  ...
  map = rospy.Subscriber("/move_base/global_costmap/costmap", OccupancyGrid, set_map)
